...
void Tlc5940::init(uint16_t initialValue)
{
  ...
  /* Timer X - BLANK / XLAT */
  TCCRXA = _BV(COMXB1);  // non inverting, output on OCXB, BLANK
  TCCRXB = _BV(WGMX3);   // Phase/freq correct PWM, ICRX top
  OCRXA = 1;             // duty factor on OCXA, XLAT is inside BLANK
  OCRXB = 2;             // duty factor on BLANK (larger than OCRXA (XLAT))
  ICRX = TLC_PWM_PERIOD; // see tlc_config.h
...
#endif
  TCCRXB |= _BV(CSX0);      // no prescale, (start pwm output)
  update();
}
